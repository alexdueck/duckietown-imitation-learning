...
def save_dataset(
    ...
    with actions_file.open("w", newline="") as file:
        ...
        for step_idx, timestamp, image_name, action in zip(
            step_indices,
            timestamps,
            image_names,
            actions,
...
def trim_initial_zero_actions(
    ...
    actions: list[np.ndarray],
    step_indices: list[int],
) -> tuple[list[np.ndarray], list[float], list[np.ndarray], list[int], int]:
...
def main():
    ...
    try:
        ...
        for step_idx in range(MAX_STEPS):
            ...
            print(
                f"{timestamps[-1]:+.3f} | step={step_idx} | "
        ...
        observations, timestamps, actions, step_indices, trimmed_initial_samples = trim_initial_zero_actions(
            observations,
            timestamps,
            actions,
            step_indices,
